document.getElementById('calculateBtn').addEventListener('click', function () {
  ...
  document.getElementById('cernium result').textContent = `${cerniumCosts.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`;
  document.getElementById('arcus result').textContent = `${arcusCosts.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`;
  document.getElementById('odium result').textContent = `${odiumCosts.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`;
  document.getElementById('total result').textContent = `${totalCosts.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}`;
});
